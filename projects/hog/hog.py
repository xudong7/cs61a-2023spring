...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # "*** YOUR CODE HERE ***"
    '''
    compute score of roll_dice
    '''
    is_one = False
    all_dicts = [dice() for i in range(num_rolls)]
    for d in all_dicts:
        if d == 1:
            is_one = True
    return 1 if is_one else sum(all_dicts)
    # END PROBLEM 1
...
def tail_points(opponent_score):
    ...
    compute tail_score 
    '''
    opponent_score, m = opponent_score // 10, opponent_score % 10
...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    # "*** YOUR CODE HERE ***"
    '''
    run tail_score if num_rolls == 0 else normal score
    '''
    total = 0
    if num_rolls != 0:
        total = total + roll_dice(num_rolls, dice)
    else:
        total = total + tail_points(opponent_score)
    return total
    # END PROBLEM 3
...
def perfect_square(score):
    '''
    return bool value
    True if score is n ^ 2 and n is int else False
    '''
    k = 1
    while k <= score:
        if k * k == score:
...
def next_perfect_square(score):
    '''
    return next score like (n+1) ^ 2 if current score is equal to n ^ 2
    '''
    k, total = 1, 1
    while k <= score:
        if k * k == score:
...
def is_always_roll(strategy, goal=GOAL):
    ...
    # BEGIN PROBLEM 7
    # "*** YOUR CODE HERE ***"
    # for each player that has higher score
    # judge the return value of strategy to examine if is_always_roll
    score0, score1 = 0, 0
    ref = strategy(score0, score1)
    while score0 < goal:
        while score1 < goal:
            if ref != strategy(score0, score1):
                return False
            score1 += 1
        score1 = 0
        score0 += 1
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    tail_score = tail_points(opponent_score)
    if tail_score >= threshold: return 0
    new_score = score + tail_score
    if perfect_square(new_score) and next_perfect_square(new_score) - new_score >= threshold:
        num_rolls = 0
    return num_rolls  # Remove this line once implemented.
    # END PROBLEM 11
